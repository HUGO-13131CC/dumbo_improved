...
# from dumbobft.core.baisedbinaryagreement_ import baisedbinaryagreement
...
def validatedagreement(sid, pid, N, f, PK, SK, PK1, SK1, PK2s, SK2, input, decide, receive, send, predicate=lambda x: True, logger=None):
    ...
    # TODO: Random permutation is time-cost, and the adversary is difficult to get the all order of the permutation
    '''
        Why?
            Only when the adversary gets the whole order of the permuted list, its attack could be valid
        How?
            Using normal permutation first, and count the ABA instances. Once the number of ABA instances
            over a constant [3/4/5], use the coin-flipping to do the random permutation, and finish the
            agreement
        Settings?
            :param int aba_num_thres: the max number of ABA instances before using the random permutation
    '''
    ...
    while True:
        ...
        # aba = gevent.spawn(baisedbinaryagreement, sid + 'ABA' + str(r), pid, N, f, coin,
        #                    aba_inputs[r].get, aba_outputs[r].put_nowait,
        #                    aba_recvs[r].get, make_aba_send(r), aba_num_thres)
        # aba.get is a blocking function to get aba output
        # if not aba.get():
        #     # TODO: Exceed the threshold and do the random permutation
        #     print(f"get: {aba.get()}")
